...
class QNetwork(nn.Module):
    def __init__(self, input_size, output_size):
        ...
        self.fc1 = nn.Linear(input_size, 32).to(device)
        self.fc2 = nn.Linear(32, 16).to(device)
        self.fc3 = nn.Linear(16, output_size).to(device)
...
model_save_path = 'q_network.pth1'
...
while True:
    ...
    for step in range(max_steps_per_episode):
        ...
        with torch.no_grad():
            q_next = q_network(torch.FloatTensor(next_state).to(device))
